...
This guide is about deliberately provoking each project's known shortcomings by hand — it's a complement to the recorded Measure step below, not a replacement for it. Every test here is still worth doing even once the Measure step is automated: they're exploratory (find a new failure mode, understand *why* something breaks), where the Measure step is a fixed, repeatable trial protocol.

## How `measurements/results.md` actually gets filled in

As of this repo's automation pass, most of this is no longer manual. Every project except 2.5 has a `measure.py` (Project 3: `verify_policy.py`) that runs the real trials this guide describes provoking by hand, logs each one's outcome **and why it failed** (not just pass/fail) to a `measurements/*.jsonl` file, and rewrites `results.md`'s tables from that log directly — see the README's "Automating the Measure step" table for the exact command per project. Two things still don't come from `measure.py` alone:

1. **The Takeaway prose isn't always auto-written.** `measure.py` fills in a mechanically-computed one-liner where it can (e.g. Project 1's retry-on-vs-off gap per model), but a real narrative synthesis — "why does this specific ablation break fastest" — still benefits from a human (or an agent) actually reading the `.jsonl` and reasoning about it, especially anywhere the auto-computed takeaway is left as a `<!-- Fill in -->` comment because the signal genuinely needs judgment, not just arithmetic.
2. **This guide's manual "break it" maneuvers are a different, complementary kind of testing.** Kill points at *specific* points chosen for what they'd expose, hand-edited `progress.json`, `mklink /J` junctions, reading the actual model transcript to see *how* it failed — these go deeper than `measure.py`'s fixed trial protocol and are how you find a new failure mode worth turning into a new measurement, not just re-running the existing one. `measure.py`'s kill points, for instance, use an in-process `KILL_AFTER_TURN` hook for determinism (a real `taskkill` can't land on a precise turn boundary), so a manual pass with an actual OS-level kill is still worth doing at least once to confirm the two produce the same result.
